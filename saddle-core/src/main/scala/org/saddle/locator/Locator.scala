...
/** The Locator class provides a mapping from a key to the first location of
  * that key within a corresponding [[org.saddle.Index]] containing at least one
  * of that key.
  *
  * As it provides a representative for a subset of keys in the index, in the
  * language of category theory, this mapping is a ''section'':
  *
...
trait Locator[@spec(Boolean, Int, Long, Double) T] {
  ...
  /** Should return the first offset corresponding to the provided key, or -1 if
    * none was found.
    * @param key
    *   The key to query
    */
...
object Locator {
  ...
  /** Factory method to create a new Locator instance.
    ...
    */
  def apply[@spec(Boolean, Int, Long, Double) C](sz: Int = 16)(implicit
      st: ST[C]
  ): Locator[C] = st.makeLoc(sz)
}
