...
/** Provides concrete implementations of binary operations for
  * [[org.saddle.Mat]]; these instances provide implicit support for
  * [[org.saddle.ops.NumericOps]] methods which are inherited by Mat.
  */
trait BinOpMat {
  // ***************
  ...
  /** Binary element-wise operation on two Mats
    *
    * Broadcasts according to the following rules (same as numpy). If any of the
    * pairs of dimensions are the same, or one of the pair is size 1, then the
    * dimensions are compatible. If the compatible dimension is size 1, then it
    * is virtually expanded to match the size of the corresponding dimension of
    * the other operand.
    */
  final class MatMatElemOp[
    ...
    def apply(v1: Mat[A], v2: Mat[B]) =
      if (v1.numRows == v2.numRows && v1.numCols == v2.numCols) {
        ...
        (v1.numRows == v2.numRows || v1.numRows == 1 || v2.numRows == 1) &&
  ...
  final class MatMatElemOpIp[
    ...
    def apply(v1: Mat[A], v2: Mat[B]) =
      if (v1.numRows == v2.numRows && v1.numCols == v2.numCols) {
        ...
        (v1.numRows == v2.numRows || v2.numRows == 1) &&
