...
object IndexMaker extends IndexMakerLowPriority {
  ...
  private[this] def zip2V[T[K] <: IndexedSeq[K], A: ORD, B: ORD](
      a: T[A],
      b: T[B]
  ): Index[(A, B)] = {
  ...
  private[this] def zip2Vec[A: ORD, B: ORD](
      a: Vec[A],
      b: Vec[B]
  ): Index[(A, B)] = {
    require(a.length == b.length, "Arguments must have same length")
  ...
  private[this] def zip3V[T[K] <: IndexedSeq[
    K
  ], A: ORD, B: ORD, C: ORD](
      a: T[A],
      b: T[B],
  ...
  private[this] def zip4V[T[K] <: IndexedSeq[
    K
  ...
  private[this] def zip5V[T[K] <: IndexedSeq[
    K
  ], A: ORD, B: ORD, C: ORD, D: ORD, E: ORD](
      a: T[A],
      b: T[B],
      c: T[C],
      d: T[D],
