...
trait Index[@spec(Boolean, Int, Long, Double) T] {
  ...
  /** A [[org.saddle.scalar.ScalarTag]] representing the kind of Scalar found in
    * this index.
    */
  ...
  /** Given a sequence of keys, return the sequence of first locations in the
    * index at which those keys correspondingly occur, ignoring keys which do
    * not exist.
    * @param keys
  ...
  /** Given a sequence of keys, return the sequence of locations in the index at
    * which those keys correspondingly occur, ignoring keys which do not exist.
    * @param keys
    *   Sequence of keys to find
    */
  ...
  /** Given an array of keys, return the sequence of locations in the index at
    * which those keys correspondingly occur, ignoring keys which do not exist.
    * @param keys
    *   Sequence of keys to find
    */
  ...
  /** Find the first location whereby inserting a key would maintain a sorted
    * index. Index must already be sorted.
    * @param t
    *   Key that would be inserted
    */
  def lsearch(t: T): Int

  /** Find the last location whereby inserting a key would maintain a sorted
    * index. Index must already be sorted.
    * @param t
    *   Key that would be inserted
    */
  ...
  /** Returns a slice of an index between two keys; if inclusive is false, then
    * exclude the upper bound. Index must be sorted, as this method relies on
    * lsearch and rsearch.
    * @param from
    *   Key lower bound
    * @param to
  ...
  /** Returns the int location of the first element of the index to satisfy the
    * predicate function, or -1 if no element satisfies the function.
    * @param pred
    *   Function from T => Boolean
    */
  def findOne(pred: T => Boolean): Int = VecImpl.findOne(toVec)(pred)(scalarTag)

  /** Returns true if there is an element which satisfies the predicate
    * function,
    * @param pred
    *   Function from T => Boolean
    */
  ...
  /** For an index which contains Tuples, drop the right-most element of each
    * tuple, resulting in a new index.
    * @param ev
    *   Implicit evidence of a Splitter instance that takes T (of arity N) to U
    *   (of arity N-1)
    * @tparam U
  ...
  /** Given this index whose elements have arity N and another index of arity 1,
    ...
    * Cartesian product of the two, in the provided order. See
    * [[org.saddle.index.Stacker]] for more details.
    * @param other
    *   Another Index
    * @param ev
  ...
  /** Given this index contains tuples of arity N > 1, split will result in a
    * pair of index instances; the left will have elements of arity N-1, and the
    * right arity 1.
    * @param ev
  ...
  /** Generates offsets into current index given another index for the purposes
    * of re-indexing. For more on reindexing, see
    * [[org.saddle.index.ReIndexer]]. If the current and other indexes are
    * equal, a value of None is returned.
    *
    * @param other
  ...
  /** Returns true if the index contains at least one entry equal to the
    * provided key
    * @param key
    *   Key to query
    */
  ...
  /** Produces a [[org.saddle.index.ReIndexer]] corresponding to the
    * intersection of this Index with another. Both indexes must have set
    * semantics - ie, have no duplicates.
    *
    * @param other
    *   The other index
    */
  ...
  /** Produces a [[org.saddle.index.ReIndexer]] corresponding to the union of
    * this Index with another. Both indexes must have set semantics - ie, have
    * no duplicates.
    *
    * @param other
    *   The other index
  ...
  /** Returns true if the ordering of the elements of the Index is
    * non-decreasing.
    */
  def isMonotonic: Boolean

  /** Returns true if the index is either unique, or any two or more duplicate
    * keys occur in consecutive locations in the index.
    */
  ...
  /** Map over the elements in the Index, producing a new Index, similar to Map
    * in the Scala collections.
    *
    * @param f
...
object Index {
  ...
  /** Factory method to create an Index; the basic use case is to construct a
    * multi-level index (i.e., an Index of Tuples) via a Tuple of Vecs.
    *
    * For instance:
    *
    * {{{
