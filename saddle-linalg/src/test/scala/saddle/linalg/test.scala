...
class VV1Suite extends AnyFunSuite {
  test("1x3") {
    ...
    assert((m1 vv m2) == (4d + 10d + 18d))
    assert((m1.slice(0, 1) vv m2.slice(0, 1)) == (4d))
    assert((Vec(1d, 2d, 3d, 4d, 5d, 6d) vv Vec(1d, 2d, 3d, 4d, 5d, 6d)) == 91)
  }
...
class DeterminantPDSuite extends AnyFunSuite {
  test("2x2") {
    ...
    assert(
      math
        .abs(
          a.determinantPD.get - a.eigenValuesSymm(100).map(math.log10).sum
        ) < 1e-10
    )
